          ...
          <div class="space-y-3">
            <div>
              ...
              <div class="flex items-center gap-2">
                <code class="flex-1 block bg-accent border border-default rounded px-2 py-2 font-mono text-xs break-all">
                  {{ keyDetail?.id ?? "-" }}
                </code>
                <button
                  type="button"
                  class="shrink-0 px-2 py-1 rounded border border-default bg-secondary hover:bg-accent text-xs"
                  @click="copy(keyDetail?.id ?? '')"
                  :disabled="!keyDetail?.id"
                >
                  Copy
                </button>
              </div>
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="shrink-0 px-2 py-1 rounded border border-default bg-secondary hover:bg-accent text-xs"
                  @click="copy(keyDetail?.secretKey ?? '')"
                  :disabled="!keyDetail?.secretKey"
                >
                  Copy
                </button>
              </div>
