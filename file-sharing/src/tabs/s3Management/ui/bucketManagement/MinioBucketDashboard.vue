<template>
  <div class="space-y-4 sm:px-4 lg:px-6 sm:rounded-lg bg-accent rounded-md border border-default">
    ...
    <div v-else-if="stats" class="space-y-4 m-4">
      ...
      <div v-if="stats.replicationEnabled && stats.replicationUsage"
        ...
        <dl class="space-y-1 text-xs text-default">
          <div class="flex justify-between gap-2">
            <dt class="text-default">Pending</dt>
            <dd class="font-medium">
          ...
          <div class="flex justify-between gap-2">
            <dt class="text-default">Failed</dt>
            <dd class="font-medium">
      ...
      <div v-if="histogramEntries.length"
        class="rounded-lg border border-default bg-default px-4 py-3 text-sm text-default">
        <p class="text-sm uppercase tracking-wide text-default font-semibold mb-2">
          Object size distribution
        </p>
        <div class="space-y-1 text-xs text-default">
          <div v-for="entry in histogramEntries" :key="entry.label" class="flex items-center gap-2">
            <div class="w-48 text-default">
              {{ entry.label }}
            </div>
