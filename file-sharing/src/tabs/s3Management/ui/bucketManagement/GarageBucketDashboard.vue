  ...
  <div class="space-y-4 sm:px-4 lg:px-6 sm:rounded-lg bg-accent rounded-md border border-default">
    ...
    <div v-else-if="stats" class="space-y-4 m-4">
      ...
      <details class="rounded-lg border border-default bg-default px-4 py-3 text-xs text-default">
        ...
        <pre
          class="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-all text-sm text-default">{{ stats.raw }}</pre>
      </details>
    </div>

    <div v-else class="mx-4 rounded-lg border border-default bg-default px-4 py-3 text-sm text-default">
      No data loaded yet.
    </div>
  </div>
</template>
