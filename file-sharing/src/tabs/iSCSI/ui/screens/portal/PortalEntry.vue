...
<script setup lang="ts">
	...
	const deletePortal = () => {
		return driver.andThen((driver) => driver.deletePortalFromTarget(props.target, props.portal))
				.map(() => emit("deletePortal"))
				.mapErr((error) => new ProcessError(`Unable to delete portal ${props.portal.address}: ${error.message}`));
	}

	const actions = wrapActions({deletePortal});

	const promptDeletion = confirmBeforeAction({header: "Confirm", body: `Delete portal ${props.portal.address}?`}, actions.deletePortal);
