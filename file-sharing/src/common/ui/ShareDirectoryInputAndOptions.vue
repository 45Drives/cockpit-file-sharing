...
import {
    InputFeedback, wrapActions, type Feedback, reportError,
    InputField,
    reportSuccess,
} from "@45drives/houston-common-ui";
import { getServer, Directory, ProcessError, Server, FileSystemNode, type CommandOptions, Path, type FilesystemMount, Command } from '@45drives/houston-common-lib';
...
const usePathInfo = (path: Ref<string>, serverResult?: ResultAsync<Server, ProcessError>, commandOptions: CommandOptions = { superuser: 'try' }) => {
    ...
    const updateSubdirSuggestions = () => {
        ...
        fsNode(path.value)
            ...
            .andThen(node =>
                new Directory(node).getChildren({ pathFilter, limit: 50 }))
...
const { exists, isDirectory, isAbsolute, mountInfo, subdirSuggestions, forceUpdatePathInfo } = usePathInfo(path);
...
const createZfsDataset = () =>
    mountInfo.value?.filesystem.type !== "zfs"
        ...
        : server.andThen(server => {
            return new Path(path.value).resolveOn(server, false)
                .andThen(path => {
                    if (!path.isAbsolute()) {
    ...
    >
        <span class="space-x-1">
            ...
            <template v-if="mountInfo?.filesystem.type === 'zfs'">
                ...
                <button
                    @click="actions.createZfsDataset"
                    class="underline"
                >
